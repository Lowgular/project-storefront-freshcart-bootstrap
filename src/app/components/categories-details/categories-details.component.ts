...
export class CategoriesDetailsComponent {
  ...
  readonly freshProducts$: Observable<FreshProductsDetailedQueryModel[]> =
    combineLatest([
      ...
      map(([freshProducts, categoryId, sortOption, filters]) => {
        const freshProductsdetails = this._filterByAdditionalFilters(
          freshProducts,
          categoryId,
          filters
        );
  ...
  private _filterByAdditionalFilters(
    freshProducts: FreshProductsModel[],
    categoryId: string,
    filters: FiltersFreshProductsQueryModel
  ): FreshProductsDetailedQueryModel[] {
    return this._filterByCategoryIdAndMapToFPQuery(
      freshProducts,
      categoryId
    ).filter((freshProduct) => {
